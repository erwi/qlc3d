...
TestSettingsFile createSettingsFile(const std::string &fileContents) {
    string fileName = std::tmpnam(nullptr);
    FILE* fid = fopen(fileName.c_str(), "wt");
    std::fputs(fileContents.c_str(), fid);
    fflush(fid);
    fclose(fid);
    return TestSettingsFile {fileName, fid};
}
...
TEST_CASE("MeshName is required in settings file") {
    auto settingsFile = createSettingsFile(""); // create empty settings file
    try {
        SettingsReader reader(settingsFile.fileName);
...
TEST_CASE("Read Simu from settings file") {
    ...
    auto settingsFile = createSettingsFile(contents);
