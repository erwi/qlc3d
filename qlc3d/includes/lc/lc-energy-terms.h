...
namespace LcEnergyTerms {
  ...
  inline void assembleThermotropic(SpaMtrix::DenseMatrix *lK,
    ...
    const unsigned int x2 = 2 * npe;
    const unsigned int x3 = 3 * npe;
    const unsigned int x4 = 4 * npe;

    for (unsigned int i = 0; i < npe; i++) {
      const double ShR = shapes.N(i) * mul;
      // RHS terms
      lL[i + x0] += (A * q1 + D3 * 0.5 * B * (q5 * q5 * rt6 * 0.5 - q3 * q3 * rt6 - rt6 * q2 * q2 + q1 * q1 * rt6 +
                                             q4 * q4 * rt6 * 0.5) + C * R * q1) * ShR;
      lL[i + x1] += (A * q2 + D3 * B * (0.75 * q5 * q5 * rt2 - q1 * rt6 * q2 - 0.75 * q4 * q4 * rt2) + C * R * q2) * ShR;
      lL[i + x2] += (A * q3 + D3 * B * (-q3 * q1 * rt6 + 1.5 * rt2 * q5 * q4) + C * R * q3) * mul * shapes.N(i);
      lL[i + x3] +=
      ...
      SpaMtrix::DenseMatrix &M = *lK;
      for (unsigned int j = 0; j < npe; j++) {
        const double ShRC = shapes.N(i) * shapes.N(j) * mul;
        // Matrix diagonal terms 11, 22, 33, 44, 55
        M(i + x0, j + x0) += (A + D3 * B * q1 * rt6 + 2.0 * C * q1 * q1 + C * R) * ShRC;
        M(i + x1, j + x1) += (A - D3 * B * q1 * rt6 + 2.0 * C * q2 * q2 + C * R) * ShRC;
        M(i + x2, j + x2) += (A - B * q1 * rt6 * D3 + 2.0 * C * q3 * q3 + C * R) * ShRC;
        M(i + x3, j + x3) += (A + B * (q1 * rt6 - 3.0 * q2 * rt2) * D6 + 2.0 * C * q4 * q4 + C * R) * ShRC;
        M(i + x4, j + x4) += (A + B * (q1 * rt6 + 3.0 * q2 * rt2) * D6 + 2.0 * C * q5 * q5 + C * R) * ShRC;

        // Matrix off-diagonal terms
        // 12
        M(i + x0, j + x1) += (-D3 * B * rt6 * q2 + 2.0 * C * q2 * q1) * ShRC;
        M(i + x1, j + x0) += (-D3 * B * rt6 * q2 + 2.0 * C * q2 * q1) * ShRC;
        // 13
        M(i + x0, j + x2) += (-B * q3 * rt6 * D3 + 2.0 * C * q3 * q1) * ShRC;
        M(i + x2, j + x0) += (-B * q3 * rt6 * D3 + 2.0 * C * q3 * q1) * ShRC;
        // 14
        M(i + x0, j + x3) += (B * q4 * rt6 * D6 + 2.0 * C * q4 * q1) * ShRC;
        M(i + x3, j + x0) += (B * q4 * rt6 * D6 + 2.0 * C * q4 * q1) * ShRC;
        // 15
        M(i + x0, j + x4) += (B * q5 * rt6 * D6 + 2.0 * C * q5 * q1) * ShRC;
        M(i + x4, j + x0) += (B * q5 * rt6 * D6 + 2.0 * C * q5 * q1) * ShRC;

        // 23
        M(i + x1, j + x2) += (2.0 * C * q3 * q2) * ShRC;
        M(i + x2, j + x1) += (2.0 * C * q3 * q2) * ShRC;
        // 24
        M(i + x1, j + x3) += (-B * q4 * rt2 * D2 + 2.0 * C * q4 * q2) * ShRC;
        M(i + x3, j + x1) += (-B * q4 * rt2 * D2 + 2.0 * C * q4 * q2) * ShRC;
        // 25
        M(i + x1, j + x4) += (B * q5 * rt2 * D2 + 2.0 * C * q5 * q2) * ShRC;
        M(i + x4, j + x1) += (B * q5 * rt2 * D2 + 2.0 * C * q5 * q2) * ShRC;

        // 34
        M(i + x2, j + x3) += (B * q5 * rt2 * D2 + 2.0 * C * q4 * q3) * ShRC;
        M(i + x3, j + x2) += (B * q5 * rt2 * D2 + 2.0 * C * q4 * q3) * ShRC;
        // 35
        M(i + x2, j + x4) += (B * q4 * rt2 * D2 + 2.0 * C * q5 * q3) * ShRC;
        M(i + x4, j + x2) += (B * q4 * rt2 * D2 + 2.0 * C * q5 * q3) * ShRC;

        // 45
        M(i + x3, j + x4) += (B * q3 * rt2 * D2 + 2.0 * C * q5 * q4) * ShRC;
        M(i + x4, j + x3) += (B * q3 * rt2 * D2 + 2.0 * C * q5 * q4) * ShRC;
      }
    }
  }

  inline void assembleElasticL1(SpaMtrix::DenseMatrix *lK,
                                std::vector<double> &lL,
                                const TetShapeFunction &shapes,
                                const double &tetDeterminant,
    ...
    for (unsigned int i = 0; i < npe; i++) {
      ...
      lL[i + x0] += (ShRx * q1x + ShRy * q1y + ShRz * q1z) * L1 * mul;
      lL[i + x1] += (ShRx * q2x + ShRy * q2y + ShRz * q2z) * L1 * mul;
      lL[i + x2] += (ShRx * q3x + ShRy * q3y + ShRz * q3z) * L1 * mul;
      lL[i + x3] += (ShRx * q4x + ShRy * q4y + ShRz * q4z) * L1 * mul;
      lL[i + x4] += (ShRx * q5x + ShRy * q5y + ShRz * q5z) * L1 * mul;
  ...
  inline void assembleChiralTerms(SpaMtrix::DenseMatrix *lK,
                                  std::vector<double> &lL,
                                  const TetShapeFunction &shapes,
                                  const double &tetDeterminant,
    ...
    for (unsigned int i = 0; i < npe; i++) {
      ...
      lL[i + x0] += mul * 0.25 * rt3 * L4 * (ShR * (q4y - q5x) + ShRx * q5 - ShRy * q4);
      lL[i + x1] += mul * L4 * (-ShR * (-0.5 * q3z + 0.25 * q4y + 0.25 * q5x) + 0.25 * ShRx * q5 + 0.25 * ShRy * q4 - 0.5 * ShRz * q3);
      lL[i + x2] += mul * L4 * (ShR * (-0.5 * q2z + 0.25 * q4x - 0.25 * q5y) - 0.25 * ShRx * q4 + 0.25 * ShRy * q5 + 0.5 * ShRz * q2);
      lL[i + x3] += mul * 0.25 * L4 * (-rt3 * ShR * q1y + ShR * q2y - ShR * q3x + ShR * q5z + ShRx * q3 + rt3 * ShRy * q1 - ShRy * q2 - ShRz * q5);
      lL[i + x4] += mul * 0.25 * L4 * (rt3 * ShR * q1x + ShR * q2x + ShR * q3y - ShR * q4z - rt3 * ShRx * q1 - ShRx * q2 - ShRy * q3 + ShRz * q4);
      ...
      for (unsigned int j = 0; j < npe; j++) {
        ...
        M(i + x0, j + x3) += mul * 0.25*rt3*L4*(-ShC*ShRy + ShCy*ShR);
        M(i + x0, j + x4) += mul * 0.25*rt3*L4*(ShC*ShRx - ShCx*ShR);

        M(i + x1, j + x2) += mul * 0.5*L4*(-ShC*ShRz + ShCz*ShR);
        M(i + x1, j + x3) += mul * 0.25*L4*(ShC*ShRy - ShCy*ShR);
        M(i + x1, j + x4) += mul * 0.25*L4*(ShC*ShRx - ShCx*ShR);

        M(i + x2, j + x1) += mul * 0.5*L4*(ShC*ShRz - ShCz*ShR);
        M(i + x2, j + x3) += mul * 0.25*L4*(-ShC*ShRx + ShCx*ShR);
        M(i + x2, j + x4) += mul * 0.25*L4*(ShC*ShRy - ShCy*ShR);

        M(i + x3, j + x0) += mul * 0.25*rt3*L4*(ShC*ShRy - ShCy*ShR);
        M(i + x3, j + x1) += mul * 0.25*L4*(-ShC*ShRy + ShCy*ShR);
        M(i + x3, j + x2) += mul * 0.25*L4*(ShC*ShRx - ShCx*ShR);
        M(i + x3, j + x4) += mul * 0.25*L4*(-ShC*ShRz + ShCz*ShR);

        M(i + x4, j + x0) += mul * 0.25*rt3*L4*(-ShC*ShRx + ShCx*ShR);
        M(i + x4, j + x1) += mul * 0.25*L4*(-ShC*ShRx + ShCx*ShR);
        M(i + x4, j + x2) += mul * 0.25*L4*(-ShC*ShRy + ShCy*ShR);
        M(i + x4, j + x3) += mul * 0.25*L4*(ShC*ShRz - ShCz*ShR);
      }
    }
  }

  inline void assembleThreeElasticConstants(SpaMtrix::DenseMatrix *lK,
                                            std::vector<double> &lL,
    ...
    for (unsigned int i = 0; i < npe; i++) {
      ...
      for (unsigned int j = 0; j < npe; j++) {
        ...
        temp += (ShC * ShRx * rt2 * q5z * D2 + ShC * ShRz * rt2 * q5x * D2 + ShCx * ShR * rt2 * q5z * D2 -
                 ...
                 ShCz * ShRx * q5 * rt2 * D2 + ShCz * ShRy * q4 * rt2 * D2 + ShCz * ShRz * q1 * rt23 * D3) * L6;
        M(i + x4, j + x4) += temp * mul;
      }
    }
  }


  inline void assembleDielectric(std::vector<double> &lL,
                                 const TetShapeFunction &shapes,
                                 const double &tetDeterminant,
                                 const double &Vx,
                                 const double &Vy,
    ...
    for (unsigned int i = 0; i < npe; i++) {
      const double ShR = shapes.N(i) * mul;
      lL[i + x0] += (rt6 * (Vx * Vx + Vy * Vy - 2.0 * Vz * Vz) * deleps * D3 * D6 * eps0) * ShR;
      lL[i + x1] += (-rt2 * (Vx * Vx - Vy * Vy) * deleps * D6 * eps0) * ShR;
      lL[i + x2] += (-rt2 * Vx * Vy * deleps * D3 * eps0) * ShR;
      lL[i + x3] += (-rt2 * Vz * Vy * deleps * D3 * eps0) * ShR;
      lL[i + x4] += (-rt2 * Vx * Vz * deleps * D3 * eps0) * ShR;
    }
  }

  inline void addWeakAnchoring(SpaMtrix::DenseMatrix *lK,
                               std::vector<double> &lL,
                               const TriShapeFunction &shapes,
    ...
    const double mul = shapes.getWeight() * triDeterminant;
    ...
    for (unsigned int i = 0; i < npe; i++) {
      const double shR = shapes.N(i) * mul;
      lL[i + x0] += shR * (S1v1 + S1v2 + T1);
      lL[i + x1] += shR * (S2v1 + S2v2 + T2);
      lL[i + x2] += shR * (S3v1 + S3v2 + T3);
      lL[i + x3] += shR * (S4v1 + S4v2 + T4);
      lL[i + x4] += shR * (S5v1 + S5v2 + T5);
