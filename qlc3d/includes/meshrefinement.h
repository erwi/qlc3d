...
class RefReg{
    ...
    */
	...
	void setType(const RefReg_Type& type){ Type = type;}
	bool setType(const std::string& type);
	void addParams(const double& P) {Params.push_back(P);}
	void addDistance(const double& D) {Distance.push_back(D);}
	...
	void addX(const double& x){X.push_back(x);}
	void addY(const double& y){Y.push_back(y);}
	void addZ(const double& z){Z.push_back(z);}
	...
	RefReg_Type getType(){return Type;}
};
...
public:
    ...
    AutoRef();
    void addMaxValue(const double& val){ MaxValue.push_back( val); }
    void addMaxValue(const std::vector<double>& vals);
    double getMaxValue(const unsigned int& iter);
    double getMinSize(){return MinSize;}
    void setMinSize(const double& val){MinSize = val;}
    bool setType( const std::string& type);
    unsigned int getNumIterations();
    void printAutoref();

    inline int getRefIter(){return RefIter;}
    inline void setRefIter(const int& iteration){RefIter = iteration;}
    inline bool isRefIter( const int& iteration){
        if ( iteration == 0 ) { return false;}       // zero RefIter = refine only at end of simu
        if (iteration % RefIter == 0) {return true;} // Returns true if this is a refinement iteration
...
public:
    EndRef();
    ~EndRef();
    int getEndRefIteration() { return EndRefIteration; }
    void incrementEndRefIteration() { EndRefIteration ++; }
};
...
class MeshRefinement
{
	...
	public:
        ...
        void incRefiter();
        unsigned int getRefiter(){return refiter;}          // is this global nth time mesh is refined??
        bool isNeedsNewMesh( ){return needs_new_mesh;}
        void setNeedsNewMesh(bool n){needs_new_mesh = n;}
