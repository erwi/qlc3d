...
double calculateBluePhaseEnergy(Geometry& geom,
                                SolutionVector& q,
                                SolutionVector& v,
                                LC& mat_par)
{
    /*!
      Calculates the total energy fusing the 'Blue Phase' formulation
      */
    ...
    const Mesh& t = *geom.t;  // REFERENCE TO TETRAHEDRA
    const double* p = const_cast<const double*>( geom.getPtrTop() );

    double Ftotal = 0;
#pragma omp parallel for reduction (+:Ftotal)
    for (idx it = 0 ; it < t.getnElements() ; it++)// FOR EACH TETRAHEDRON
    {
        if ( t.getMaterialNumber(it) != MAT_DOMAIN1 )
            continue;


        // LOCAL CACHED COPY OF CURRENT ELEMENT
        ...
        for (idx igp = 0 ; igp < shapes.ngp ; ++igp) // FOR EACH GAUSS POINT
        {
    ...
    #define IND(i,j) 5*(i) + (j)
            ...
            double Felastic(0);
            double K2 = K1;
            ENERGY_ELASTIC_2K(Felastic);

            double Fthermotropic=A*(R)/2.0 +
            ...
            Ftotal += mul*( Felastic + Fthermotropic );

        }// end for igp
    }// end for each tetrahedron it
