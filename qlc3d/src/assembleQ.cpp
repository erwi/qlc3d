	    ...
	    //ADD TERMS TO RHS VECTOR
            ...
            for (int j=0 ; j<4 ; ++j)
            {
                ...
                if (L4!=0)
                {
                    ...
                    temp = (ShRx*rt3*ShC*D4-ShR*rt3*ShCx*D4)*L4;
                    lK[i   ][j+12] += temp;
                    lK[i+12][j+0 ] -= temp;
                    //Kc[0][4] = (-ShRy*rt3*ShC/4.0+ShR*rt3*ShCy/4.0)*L4;
                    temp = (-ShRy*rt3*ShC*D4+ShR*rt3*ShCy*D4)*L4;
                    lK[i   ][j+16] += temp;
                    lK[i+16][j   ] -= temp;
                    //Kc[1][2] = (-ShRz*ShC/2.0+ShR*ShCz/2.0)*L4;
                    temp = (-ShRz*ShC/2.0+ShR*ShCz/2.0)*L4;
                    lK[i+4][j+8] += temp;
                    lK[i+8][j+4] -= temp;
                    //Kc[1][3] = (ShC*ShRx/4.0-ShCx*ShR/4.0)*L4;
                    temp = (ShC*ShRx*D4-ShCx*ShR*D4)*L4;
                    lK[i+4 ][j+12] += temp;
                    lK[i+12][j+4 ] -= temp;
                    //Kc[1][4] = (ShC*ShRy/4.0-ShCy*ShR/4.0)*L4;
                    temp = (ShC*ShRy*D4-ShCy*ShR*D4)*L4;
                    lK[i+4 ][j+16] += temp;
                    lK[i+16][j+4 ] -= temp;
                    ...
                    temp = (ShRz*ShC*D4-ShR*ShCz*D4)*L4;
                    lK[i+12][j+16] += temp;
                    lK[i+16][j+12] -= temp;
