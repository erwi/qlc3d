...
void ReadGiDMesh3D(Simu* simu,double **p, int *np, int **t, int *nt,int **e, int *ne, int **matt, int **mate)
{
	...
	fin.open(filename.c_str() );//.c_str(),ifstream::in);
        ...
        if (!fin.good() ){//(fin.fail())
            //printf ("File %s could not be opened, bye!\n", filename.c_str());
            //cout << ", but failed miserably - bye!" << endl;
...
//ALLOCATE MEMORY FOR MESH DATA ---- use of double pointers
	...
	int *dt			= (int*)  malloc(4*nt[0]*sizeof(int));
	int *de 		= (int*)  malloc(3*ne[0]*sizeof(int));

	int *dmatt 		= (int*)malloc(nt[0]*sizeof(int));
	int *dmate 		= (int*)malloc(ne[0]*sizeof(int));

	int* pr=NULL;
	if (nperi>0)
		pr 	= (int*)malloc(nperi*6*sizeof(int));
...
void readBinaryMesh(std::string filename,
    ...
    tmat = (int*)  malloc( *nt*sizeof(int) );
    e    = (int*)  malloc( 3**ne*sizeof(int) );
    emat = (int*)  malloc( *ne*sizeof(int) );
